...
#include "HysteresisScrollingMap.h"
#include <task/Task.h>
#include <unistd.h>
#include <stdio.h>
...
class UnitTestTask : public Task
{
    public:
        UnitTestTask(const std::string &taskName) :Task(taskName){}
        ~UnitTestTask() {}

        virtual bool initialize(void) {return true;}
        virtual bool executive(void) {return true;}
        virtual void cleanup(void) {}
};

Task* Task::get()
{
    static UnitTestTask task(string("UnitTestTask"));
    return &task;
}
...
{
    HysteresisScrollingMap<double> map;
    bool isValid;
    double c;
    RecPoint2D index;
    ...
    HysteresisScrollingMap<double>::iterator smit;
...
{
    HysteresisScrollingMap<double> testMap(50,0.25,RC,0.13579);
    ...
    HysteresisScrollingMap<double>::iterator it;;
    double val = 0;
    ...
    HysteresisScrollingMap<double> restoreMap;
...
void RCTest(void)
{
    ...
    HysteresisScrollingMap<double> testMap(range,cellRes,RC,voided);
...
void lineTest(void)
{
    ...
    double range(60), cellRes(0.25), voided(-1.0);      // map needs to be large enough to cover the range in chris's tests
    bool RC(true), isValid(false);

    HysteresisScrollingMap<double> testMap(range,cellRes,RC,voided);
    HysteresisScrollingMap<double>::LineIterator lit;
    HysteresisScrollingMap<double>::iterator ii;
    ...
    for(lit= testMap.beginLine(RecPoint2D(15,3),
        ...
        ++lit, ++value)
    {    
        BOOST_CHECK_EQUAL((*lit).first,value);
    ...
    value = 17;
    point.y = 7;
    for(point.x =15; point.x >= -30; point.x-=0.25, ++value)
    {
        testMap.set(point,value);
    }
    ...
    value = 15;
    point.x = 35;
    for (point.y = 25; point.y<=36; point.y+=0.25, ++value)
        testMap.set(point,value);
    ...
    HysteresisScrollingMap<double> testMap1(range,cellRes,RC,voided);
 
    RecPoint2D startPt( 4, 4);
    RecPoint2D endPt(8.9, 8.9);
    // fill in the map so that we're all in one location.
    for(HysteresisScrollingMap<double>::iterator ii(testMap1.begin(startPt, endPt)); ii != testMap1.end(); ++ii) 
    {
        ii.set(24);
    }
    for(HysteresisScrollingMap<double>::iterator ii(testMap1.begin(startPt, endPt)); ii != testMap1.end(); ++ii) 
    {
        BOOST_CHECK_EQUAL((*ii).first,24);
        BOOST_CHECK_EQUAL((*ii).second,true);
    }
...
void iteratorTest(void)
{
    ...
    double range(5.0), cellRes(0.25);      // map needs to be large enough to cover the range in chris's tests
    bool RC(true), talky(false);
    RecPoint2D point;
    

    // test that an initial map can be iterated over without crashing...
    unsigned char voidedByte(0);
    HysteresisScrollingByteMap testMap1( 100, 0.25, RC, voidedByte);
    HysteresisScrollingByteMap::iterator ii;
    ...
    for(ii = testMap1.begin(startPt, endPt); ii != testMap1.end(); ++ii) 
    {
        // this check should actually return invalid, but it shouldn't crash
        BOOST_CHECK_EQUAL((*ii).second, false);
    }
    // check other method for checking isValid.
    for(ii = testMap1.begin(startPt, endPt); ii != testMap1.end(); ++ii) 
    {
        BOOST_CHECK_EQUAL(ii.isValid(),0);
    }
    for(ii = testMap1.begin(startPt, endPt); ii != testMap1.end(); ++ii) 
    {
        ii.set(24);
    ...
    for(ii = testMap1.begin(startPt, endPt); ii != testMap1.end(); ++ii) 
    {
        BOOST_CHECK_EQUAL((*ii).first,24);
        BOOST_CHECK_EQUAL((*ii).second,true);
        BOOST_CHECK_EQUAL(ii.isValid(),1);
    }

    unsigned char booga(1);
    HysteresisScrollingMap<double> testMap(range,cellRes,RC,booga);
    HysteresisScrollingMap<double>::iterator jj;
    ...
    for(jj = testMap.begin(bigStartPt, bigEndPt); jj != testMap.end(); ++jj)
    {
        jj.set(-99.0);
    }
    RecPoint2D smallStartPt(2, 2);
    RecPoint2D smallEndPt(2.9, 2.9);
    for(jj = testMap.begin(smallStartPt, smallEndPt); jj != testMap.end(); ++jj)
    {
        jj.set(67);
    }
    ...
    for(jj = testMap.begin(smallStartPt, smallEndPt); jj != testMap.end(); ++jj)
    {
        BOOST_CHECK_EQUAL((*jj).first, 67);
        BOOST_CHECK_EQUAL((*jj).second,true);
    }

    RecPoint2D regionEndPt(1.75,4.9);
    for(jj = testMap.begin(bigStartPt, regionEndPt); jj != testMap.end(); ++jj)
    {
        BOOST_CHECK_EQUAL((*jj).first, -99);
        BOOST_CHECK_EQUAL((*jj).second,true);
    }
    ...
    for(jj = testMap.begin(outsideStart, outsideEnd); jj != testMap.end(); ++jj)
    {
        BOOST_CHECK_EQUAL(jj.isValid(),0);
    }
}
...
void boundingBoxTest(void)
{
    ...
    double range(10.0), cellRes(0.5), voided(-1.0);
    bool RC(true), talky(false), isValid(false);
    RecPoint2D minPt(0,0), maxPt(0,0);
    ...
    if(talky == 1)  
    {
        cout << "BoundingBox:  check initial box" << endl;
    }
    ...
    if(talky == 1)
    {
        cout << "BoundingBox:  check partially filled box" << endl;
    }
    ...
    if(talky == 1)
    {
        cout << "BoundingBox:  check fully filled box" << endl;
    }
...
void simpleHysteresisTest(const bool & useRC)
{
    cout << "Testing simple hysteresis";
    if(useRC)
    {
        cout << " with row-column scrolling";
    }
    cout << " . . ." << endl;
    //Create a map with a 5 second lifetime.
    HysteresisScrollingMap<double> testMap(50, 0.25, useRC, 0.0);
    HysteresisScrollingMap<double>::iterator ii;

    cout << "Testing LifeTime get/set . . ." << endl;
    testMap.setLifeTime(0, 500);
    boost::posix_time::time_duration lifetime(testMap.getLifeTime());

    cout << "Testing initial sets are correct . . ." << endl;
    RecPoint2D startPt(4, 4);
    RecPoint2D endPt(8.9, 8.9);
    // fill in the map so that we're all in one location.
    for(ii = testMap.begin(startPt, endPt); ii != testMap.end(); ++ii) 
    {
        ii.set(24);
    }
    for(ii = testMap.begin(startPt, endPt); ii != testMap.end(); ++ii) 
    ...
    cout << "Waiting for expiration, then testing for points being invalidated by time . . ." << endl;
    usleep(500*1000);
...
void simpleHysteresisRCTest(void)
{
    simpleHysteresisTest(true);
}

void simpleHysteresisNonRCTest(void)
{
    simpleHysteresisTest(false);
}
...
test_suite * init_unit_test_suite(int argc, char *argv[]) 
{
    ...
    test->add(BOOST_TEST_CASE(&simpleHysteresisNonRCTest), 0);
    test->add(BOOST_TEST_CASE(&simpleHysteresisRCTest), 0);
    return test;
}
