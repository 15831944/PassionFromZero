 ...
 *
 * Revision 1.2  2004/07/26 20:56:20  bode
 ...
 * Adding recGeometry.
 *
 * Revision 1.6  2003/03/11 19:33:14  bode
 * Updated constructors
 *
 * Revision 1.5  2003/03/04 21:03:57  bode
 * Added quaternion class and extended support for the pose and transform
 * classes
 *
 * Revision 1.4  2003/02/18 17:13:34  bode
 ...
 *
 * Revision 1.3  2003/02/14 22:13:33  bode
 * Radians in Serializable Pose and Transform should write and
 * read themselves properly now.
 *
 *
 *****************************************************************************/
 ...
 *****************************************************************************/
RecPose3D::RecPose3D(EulerAngleOrder 
                     order):x(0.0), y(0.0), z(0.0), 
 ...
 *****************************************************************************/
RecPose3D::RecPose3D(double ix, double iy, double iz, RecRadians r1, 
										 RecRadians r2, RecRadians r3, 
                     EulerAngleOrder order):x(ix), y(iy), z(iz), 
 ...
 *****************************************************************************/
RecPose3D::RecPose3D(const 
                     RecPose3D& b):x(b.x), y(b.y), z(b.z), rot1(b.rot1), 
                     rot2(b.rot2), rot3(b.rot3), angleOrder(b.angleOrder) { }
 ...
 *****************************************************************************/
RecPose3D::RecPose3D(const RecTransform3D& trans, 
                     EulerAngleOrder order):x(trans.matrixPP[0][3]), 
                     y(trans.matrixPP[1][3]), z(trans.matrixPP[2][3]), 
                     angleOrder(order)
{
  double sb;
	double cb;
  double k1;
	...
	{
	case XYX:
    k1 = sqrt((trans.matrixPP[0][1]*trans.matrixPP[0][1]) + 
      (trans.matrixPP[0][2]*trans.matrixPP[0][2]));
    ...
    else
    {
      ...
      rot3 = atan2((trans.matrixPP[0][1]/sb), (trans.matrixPP[0][2]/sb));
		...
		break;
	case XYZ:
    k1 = sqrt((trans.matrixPP[0][0]*trans.matrixPP[0][0]) + 
      (trans.matrixPP[0][1]*trans.matrixPP[0][1]));
    ...
    else 
    {
      ...
      rot3 = atan2((trans.matrixPP[0][2]/cb), (trans.matrixPP[0][0]/cb));
		...
		break;
	case YXY:
    k1 = sqrt((trans.matrixPP[1][0]*trans.matrixPP[1][0]) + 
      (trans.matrixPP[1][2]*trans.matrixPP[1][2]));
	...
	case YXZ:
    k1 = sqrt((trans.matrixPP[1][0]*trans.matrixPP[1][0]) + 
      (trans.matrixPP[1][1]*trans.matrixPP[1][1]));
    ...
    else 
    {
      ...
      rot3 = atan2((trans.matrixPP[1][0]/cb), (trans.matrixPP[1][1]/cb));
		...
		break;
	case YZX:
    k1 = sqrt((trans.matrixPP[1][1]*trans.matrixPP[1][1]) + 
      (trans.matrixPP[1][2]*trans.matrixPP[1][2]));
	...
	case YZY:
		...
		break;
	case ZXY:
    k1 = sqrt((trans.matrixPP[2][0]*trans.matrixPP[2][0]) + 
      (trans.matrixPP[2][2]*trans.matrixPP[2][2]));
	...
	case ZXZ:
		k1 = sqrt((trans.matrixPP[2][0]*trans.matrixPP[2][0]) + 
			(trans.matrixPP[2][1]*trans.matrixPP[2][1]));
	...
	case ZYX:
		k1 = sqrt((trans.matrixPP[0][0]*trans.matrixPP[0][0]) + 
			(trans.matrixPP[1][0]*trans.matrixPP[1][0]));
...
ostream& operator<<(ostream& out, const RecPose3D& a)
{
  ...
  {
    return out << "(x,y,z,r1,r2,r3) = ( " << a.x << ", " << a.y << ", " << a.z 
      << ", " << a.rot1 << ", " << a.rot2 << ", " << a.rot3 << " )";
  }
}
 ...
 *              returns the result  
 *****************************************************************************/
RecDifferentialPose3D operator+(const RecDifferentialPose3D& a, 
 ...
 *****************************************************************************/
bool operator==(const RecDifferentialPose3D& a, 
                const RecDifferentialPose3D& b)
{
