...
RecPoint2D operator*(const RecTransform2D& tr, const RecPoint2D& pt)
{
  RecPoint2D newPt;
  newPt.x =
    tr.matrixPP[0][0]*pt.x + tr.matrixPP[0][1]*pt.y + tr.matrixPP[0][2];
  newPt.y =
    tr.matrixPP[1][0]*pt.x + tr.matrixPP[1][1]*pt.y + tr.matrixPP[1][2];
  return newPt;
}
...
{
	return(
		(fabs(a.matrixPP[0][0] - b.matrixPP[0][0]) < RecTransform2D::tolerance) &&
