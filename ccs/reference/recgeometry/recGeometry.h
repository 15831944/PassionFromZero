#ifndef REC_GEOMETRY_H
#define REC_GEOMETRY_H

/*****************************************************************************
 *
 * REC Geometry Core Tech Library
 *
 * FILE:	recGeometry.h
 * VERSION:	$Revision: 1.1 $  $Date: 2005/02/07 17:59:21 $
 * DESCRIPTION: public header file for recGeometry library
 * AUTHORS:	Michael Bode, Stephan Roth, Patrick Rowe and a cast of 1000s
 *
 *                           ******************************
 *                           *      Copyright (C) 2003    *
...
public:
  ...
  /*!
  \name Constructors
  Create a RecRadians object.  Values are scaled between -PI and PI when necessary
  */
	RecRadians(void); //!< default constructor which sets radian value to zero
	RecRadians(double rad); //!< constructs a RecRadians object from a double.  Scales the double when necessary.
	RecRadians(const RecRadians& rad); //!< copy constructor
  //@}

  //@{
  /*!
  \name Arithmetic And Comparison Operators
  Operators that support mathematical operations and comparisons of radians objects
  */
...
public:
  ...
  */
  ...
  friend RecPoint2D operator+(const RecPoint2D& a,
			       const RecPoint2D& b); //!< Point Addition
...
class RecVector2D
{
  public:
    ...
    */
    ...
    /*!
      \name Arithmetic Operators
      Methods for operating on 2D vectors
    */
    ...
    // inner dot product
    friend double operator*(const RecVector2D& a, const RecVector2D& b); //!< Dot Product of Vectors
    //@}
...
public:
  ...
  */
  ...
  friend RecVector3D operator-(const RecVector3D& a, const RecVector3D& b); //!< Vector Subtraction
	...
	// inner dot product
	friend double operator*(const RecVector3D& a, const RecVector3D& b); //!< Vector Dot Product
  // outer cross product
...
public:
  ...
  */
  ...
  /*!
  \name Arithmetic Operators
  Uses of a pose 2D object as an operator
  */
  ...
  friend RecPose2D operator+(const RecPose2D& a,
		const RecDifferentialPose2D& b); //!< Adds a differential pose to the passed pose and returns the resultant pose
  friend RecPose2D operator+(const RecDifferentialPose2D& a,
		const RecPose2D& b); //!< Adds a differential pose to the passed pose and returns the resultant pose
  friend RecPose2D operator-(const RecPose2D& a,
		const RecDifferentialPose2D& b);  //!< Subtracts a differential pose to the passed pose and returns the resultant pose
...
public:
  ...
  RecRadians rot3;
	...
	// Class Scope Enumerated type
  enum EulerAngleOrder{ XYX, XYZ, XZX, XZY, YXY, YXZ, YZX, YZY,
    ZXY, ZXZ, ZYX, ZYZ };
  EulerAngleOrder angleOrder;

  // Constructors
  RecPose3D(EulerAngleOrder order = RecPose3D::ZYX);
  RecPose3D(double ix, double iy, double iz, RecRadians r1, RecRadians r2,
		RecRadians r3, EulerAngleOrder order = RecPose3D::ZYX);
  ...
  // Friend Arithmetic Operators
  friend RecPose3D operator+(const RecDifferentialPose3D& a,
		const RecPose3D& b);
  friend RecPose3D operator+(const RecPose3D& a,
		const RecDifferentialPose3D& b);
  friend RecPose3D operator-(const RecDifferentialPose3D& a,
		const RecPose3D& b);
...
public:
  ...
  RecDifferentialPose3D(double inx, double iny, double inz, RecRadians inr,
		RecRadians inp, RecRadians inw);
  ...
  // Friend Arithmetic Operators
  friend RecDifferentialPose3D operator+(const RecDifferentialPose3D& a,
    const RecDifferentialPose3D& b);
  friend RecDifferentialPose3D operator-(const RecDifferentialPose3D& a,
    const RecDifferentialPose3D& b);
  friend bool operator==(const RecDifferentialPose3D& a,
    const RecDifferentialPose3D& b);
  friend bool operator!=(const RecDifferentialPose3D& a,
    const RecDifferentialPose3D& b);
...
class RecTransform2D {
  public:
    ...
    friend bool operator==(const RecTransform2D& a,
                           const RecTransform2D& b);
    friend bool operator!=(const RecTransform2D& a,
                           const RecTransform2D& b);
...
public:
  ...
  // Constructors
  RecTransform3D(void);
  RecTransform3D(const RecVector3D& vX, const RecVector3D& vY,
    const RecVector3D& vZ);
  RecTransform3D(const RecVector3D& vX, const RecVector3D& vY,
    const RecVector3D& vZ, const RecVector3D& origin);
  ...
  friend RecPoint2D operator*(const RecTransform3D& tr, const RecPoint2D& pt);
	...
	friend bool operator==(const RecTransform3D& a,
	const RecTransform3D& b);
  friend bool operator!=(const RecTransform3D& a,
	const RecTransform3D& b);
...
public:
    ...
    RecLineSegment2D(const RecPoint2D & base, const RecVector2D & direction,
                     double length, bool needsNormalize=true);
    ...
    int getIntersection(const RecLineSegment2D& l2, RecPoint2D &inter,
  ...
  protected:
    ...
    template<class Archive> void serialize(Archive &ar, unsigned int version) {ar &p1_ & p2_ & vv_;};
...
public:
  ...
  RecAxisAlignedBox3D();
  RecAxisAlignedBox3D(double minX, double maxX, double minY, double maxY,
...
class RecPolygon2D {
  ...
  private:
    ...
    int isConvexCorner(const RecPoint2D& p1, const RecPoint2D& p2,
                       const RecPoint2D& p3);
    ...
    angleToVertexP()
{
    ...
    if(verticesP.size() > 0 &&
       verticesP[0] != verticesP[verticesP.size() - 1])
    {
